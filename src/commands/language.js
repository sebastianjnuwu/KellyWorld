...
const KellyWorld = async (client, interaction, lang) => {
  ...
  await client.db.guild.findOneAndUpdate({ _id: interaction.guild.id }, { $set: { lang: value } });
  
  if (value == 1) return interaction.reply({ content: `(^^) Você fala este idioma não é? legal!`, ephemeral: true });
  
  if (value == 2) return interaction.reply({ content: `(^^) Hablas este idioma, ¿no? ¡Fresco!`, ephemeral: true });
  
  if (value == 3) return interaction.reply({ content: `(^^) You speak this language don't you? Cool!`, ephemeral: true });
